...
export default function WhatsAppFloat({ settings }: { settings: SiteSettings }) {
  ...
  return (
    <div className="fixed bottom-20 right-5 md:bottom-5 z-40 flex flex-col items-end gap-3">
      {open && (
        <div className="bg-white dark:bg-graphite dark:border-white/10 chamfer border border-line shadow-2xl w-64 p-4">
          ...
          <div className="space-y-2">
            {contacts.map((c) => (
              <a
                ...
                className="flex items-center justify-between w-full h-10 px-3 bg-[#25D366] text-white text-sm font-medium chamfer-sm hover:brightness-95 transition"
              >
                <span>{c.name}</span>
